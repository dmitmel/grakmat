...
:json-example-link:    {sources-prefix}/org/drimachine/grakmat/JSON.kt
...
https://wikipedia.org/wiki/EBNF[EBNF] grammars and contains line/column error reporting.
...
| `a atLeast(n)`                | Just like `repeat`, but match *at least* `n` times.
| `a inRange(start..end)`       | Just like `repeat`, but match **any count from `start` to `end`**.
